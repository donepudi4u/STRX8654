...
/**
 * Editor for use with data tables.  Simply call editor.edit(event, cell), passing the event and cell within the data table to edit.
 */
var editor = {};
(function() {
    //Array of undos
    ...
    editor.edit = function(event, cell, callback) {
        var input = cell.find("input");
        //var row = cell.parent();
        //var clickedRow = typesTable.row( tr );
        // console.log("inside editor row = " + clickedRow);
         //console.log("inside uielements - at editor.edit input length =" +  input.length);
        if (input.length===0) {
            if (!event.ctrlKey) {
                //alert("inside commit");
                commitValue();
               ...
               var text = cell.text();
               // alert("editor.edit cell text = " + text);
                ...
                if ($.type(origVal)==="undefined") {
                   ...
                   // alert("editor.edit celldata origVal  = " + text);
                }
     ...
     */
    editor.register = function(callback) {
     ...
     */
    editor.unregister = function(callback) {
        for (var ii=0;ii<lcallback.length;ii++) {
            if (lcallback[ii]===callback) {
    ...
    editor.addUndo = function(undo) {
        
        lundo[lundo.length] = undo;
        //console.log("inside editor.addUndo undo =" + undo);
        lredo.length = 0;
        callCallback();
    };
    ...
    editor.undo = function() {
        //console.log("inside editor.undo on button click");
        if (lundo.length!==0) {
            rollbackValue();
    ...
    /**
     * Clear editor, removing all undo and redo edits.
     */
    editor.clear = function() {
        //alert("clearing the editor");
        lundo.length = 0;
        lredo.length = 0;
        callCallback();
    };
    ...
    function commitValue() {
        if (inputWrapper.is(":visible")) {
            ...
            //var table = cell.parents(".dataTable").last().DataTable();
             //cell.parent().closest('tr').next('tr').find('input[type=text]').focus();
                    //console.log("click event seen, row = " + clickedRow[0]);
        }
    }
...
//        var offset = $("implant_main").offset().left;
//        alert("offset=" + offset);
//        var tableViewportOffsetLeft = offset - $(document).scrollLeft();
//         alert("tableViewportOffsetLeft = " + tableViewportOffsetLeft);
        ...
        //$(event.target).closest('td').next('td').find('input[type=text]').focus();
    });
