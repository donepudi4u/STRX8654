...
/**
 * Editor for use with data tables.  Simply call pcompeditor.edit(event, cell), passing the event and cell within the data table to edit.
 */
var pcompeditor = {};
(function() {
    //Array of undos
    ...
    pcompeditor.edit = function(event, cell, callback) {
        var input = cell.find("input");
        //var row = cell.parent();
        //var clickedRow = typesTable.row( tr );
        // console.log("inside pcompeditor row = " + clickedRow);
         //console.log("inside uielements - at pcompeditor.edit input length =" +  input.length);
        if (input.length===0) {
            if (!event.ctrlKey) {
                //alert("inside commit");
                pcompcommitValue();
               ...
               var text = cell.text();
               // alert("pcompeditor.edit cell text = " + text);
                ...
                if ($.type(origVal)==="undefined") {
                   ...
                   // alert("pcompeditor.edit celldata origVal  = " + text);
                }
     ...
     */
    pcompeditor.register = function(callback) {
     ...
     */
    pcompeditor.unregister = function(callback) {
        for (var ii=0;ii<lcallback.length;ii++) {
            if (lcallback[ii]===callback) {
    ...
    pcompeditor.addUndo = function(undo) {
        
        lundo[lundo.length] = undo;
        //console.log("inside pcompeditor.addUndo undo =" + undo);
        lredo.length = 0;
        callCallback();
    };
     ...
     */
    pcompeditor.undo = function() {
        //console.log("inside pcompeditor.undo on button click");
        if (lundo.length!==0) {
            rollbackValue();
            console.log("pcomp lundo[0] = " + lundo[0]); // the console.log keeps the x.undo() below from errors = ???
    ...
    /**
     * Clear pcompeditor, removing all undo and redo edits.
     */
    pcompeditor.clear = function() {
        //alert("clearing the pcompeditor");
        lundo.length = 0;
        lredo.length = 0;
        callCallback();
    };
            ...
            case keyCode.TAB:
               ...
               // horizontal scrollbar and the size of the accordion object to the table it contains to move the inner horizontal scrollbar
                ...
                if( td.parents("td:not(.noteditable)") ){
                    pcompeditor.edit(event, td, function() {});
